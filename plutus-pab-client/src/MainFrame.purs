...
import MonadApp (class MonadApp, activateContract, getFullReport, invokeEndpoint, runHalogenApp, getContractDefinitions, getContractInstanceClientState, getContractInstanceStatus)
...
handleAction LoadFullReport = do
  ...
  contractInstances <- view (RemoteData._Success) <$> getContractInstanceClientState
...
getMatchingSignature (ContractInstanceClientState { cicDefintion }) (ContractSignatures { unContractSignatures }) = find isMatch unContractSignatures
  where
  isMatch (ContractSignatureResponse { csrDefinition }) = csrDefinition == cicDefintion
